...
function EnterpriseServiceManage() {
  const navigate = useNavigate();
  const [serviceList, setServiceList] = useState([
    { id: 1, title: '园区企业一站式服务平台正式上线', category: '平台服务', publishDate: '2024-01-12', status: '已发布', image: 'https://via.placeholder.com/300x200/0ea5e9/ffffff?text=平台服务' },
    { id: 2, title: '知识产权申报与维权服务指南', category: '知识产权', publishDate: '2024-01-08', status: '已发布', image: 'https://via.placeholder.com/300x200/6366f1/ffffff?text=知识产权' },
    { id: 3, title: '法律咨询与财税服务专场活动', category: '法务财税', publishDate: '2024-01-05', status: '草稿', image: 'https://via.placeholder.com/300x200/14b8a6/ffffff?text=法务财税' },
  ]);
  ...
  const categories = ['平台服务', '知识产权', '法务财税', '人力资源', '资质认定'];
  ...
  const handleDelete = (id) => {
    if (window.confirm('确定要删除这条企业服务内容吗？')) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow-md mb-6 p-4">
          <div className="flex items-center gap-4">
            ...
            <span className="text-gray-600">企业服务管理</span>
          </div>
        </div>

        <div className="bg-white rounded-lg shadow-md p-8">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-800">企业服务管理</h1>
            <button onClick={handleAdd} className="bg-secondary-600 text-white px-6 py-2 rounded-lg hover:bg-secondary-700 transition-colors">
              + 添加内容
            </button>
          </div>

          <div className="mb-6 flex gap-4">
            <select className="border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-secondary-500">
            ...
            <input type="text" placeholder="搜索标题..." className="flex-1 border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-secondary-500" />
          </div>
              ...
              <div key={item.id} className="flex items-center gap-6 border-b border-gray-200 pb-4 last:border-0 hover:bg-gray-50 p-4 rounded-lg transition-colors">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-xs bg-secondary-100 text-secondary-700 px-2 py-1 rounded">{item.category}</span>
          ...
          {serviceList.length === 0 && (
            <div className="text-center py-12 text-gray-500">暂无企业服务内容，点击"添加内容"开始添加</div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200">
              <h2 className="text-2xl font-bold text-gray-800">{editingItem ? '编辑企业服务内容' : '添加企业服务内容'}</h2>
            </div>
            <form onSubmit={handleSubmit} className="p-6">
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" required value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                    className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-secondary-500" placeholder="请输入标题" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">分类 *</label>
                    <select required value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })}
                      className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-secondary-500">
                      {categories.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                    </select>
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <input type="text" value={formData.image} onChange={(e) => setFormData({ ...formData, image: e.target.value })}
                      className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-secondary-500" placeholder="请输入图片URL或上传图片" />
                    <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <textarea value={formData.content} onChange={(e) => setFormData({ ...formData, content: e.target.value })} rows={8}
                    className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-secondary-500" placeholder="请输入详细内容..." />
                </div>
              </div>
              <div className="flex justify-end gap-4 mt-6 pt-6 border-t border-gray-200">
                ...
                <button type="submit" className="px-6 py-2 bg-secondary-600 text-white rounded-lg hover:bg-secondary-700 transition-colors">{editingItem ? '保存修改' : '添加内容'}</button>
              </div>
            </form>
          </div>
        </div>
...
export default EnterpriseServiceManage;
