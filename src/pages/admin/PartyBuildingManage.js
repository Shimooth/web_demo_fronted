...
function PartyBuildingManage() {
  const navigate = useNavigate();
  const [partyBuildingList, setPartyBuildingList] = useState([
    { 
      id: 1, 
  ...
  const categories = ['党建活动', '党建动态', '理论学习', '组织建设'];
  ...
  const handleDelete = (id) => {
    if (window.confirm('确定要删除这条党建内容吗？')) {
      setPartyBuildingList(partyBuildingList.filter(item => item.id !== id));
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (editingItem) {
      // 更新
      setPartyBuildingList(partyBuildingList.map(item => 
      ...
      setPartyBuildingList([...partyBuildingList, newItem]);
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow-md mb-6 p-4">
          <div className="flex items-center gap-4">
            ...
            <span className="text-gray-600">园区党建管理</span>
          </div>
        </div>

        <div className="bg-white rounded-lg shadow-md p-8">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-800">园区党建管理</h1>
            <button 
              onClick={handleAdd}
          ...
          <div className="space-y-4">
            {partyBuildingList.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-6 border-b border-gray-200 pb-4 last:border-0 hover:bg-gray-50 p-4 rounded-lg transition-colors"
              >
                {/* 图片预览 */}
                <div className="flex-shrink-0">
                  <img
                    ...
                    onError={(e) => {
                      e.target.src = 'https://via.placeholder.com/128x96/cccccc/666666?text=图片加载失败';
                    }}
                  />
                </div>
          ...
          {partyBuildingList.length === 0 && (
            <div className="text-center py-12 text-gray-500">
              <p>暂无党建内容，点击"添加内容"开始添加</p>
            </div>
          )}
        </div>
      </div>
...
export default PartyBuildingManage;
